...
export default function EmployerJobsScreen() {
  ...
  return (
    <View style={styles.container}>
      <CreateJobModal visible={showModal} onClose={() => setShowModal(false)} onSuccess={fetchJobs} initialCompany={user?.companyName || ''} />
      <View style={styles.headerRow}>
        <Text style={{ fontSize: 24, fontWeight: 'bold', color: '#1e293b' }}>My Jobs</Text>
      ...
      {loading ? (
        ...
        <FlatList 
          ...
          ListEmptyComponent={<Text style={styles.emptyText}>You haven't posted any jobs yet.</Text>}
        />
      )}
    </View>
  );
...
const styles = StyleSheet.create({
  ...
  salary: { fontSize: 14, fontWeight: 'bold', color: '#10b981' },
